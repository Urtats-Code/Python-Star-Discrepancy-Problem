"""
plot_experiments.py - Visualizations for the DSM-EDA grid-sweep results.

Produces four types of plots:
  1. Best fitness by n (bar chart comparing all 8 configs)
...
Usage:
    ...
    python plot_experiments.py --csv results/experiments_dim2.csv --log-dir logs/experiments --out-dir plots/experiments
"""
import argparse
import os
...
def plot_bar_by_n(df, out_dir):
    ...
    configs  = df[["learning_method", "warm_start", "svd_noise"]].drop_duplicates()
    ...
    for ax, n in zip(axes, n_values):
        ...
        bars = ax.bar(range(len(sub)), sub["best_fitness"], color=colours, edgecolor="black", linewidth=0.5)
...
def plot_convergence(df, log_dir, out_dir):
    n_values = sorted(df["n"].unique())
    ...
    for ax, n in zip(axes, n_values):
        ...
        for _, row in sub.iterrows():
            ...
            ax.plot(h5["generations"], h5["best_fitness"],
                    ...
                    color=colour, label=label, linewidth=1.2)
        ax.set_title(f"n = {n}", fontsize=11)
...
def plot_diversity(df, log_dir, out_dir):
    n_values = sorted(df["n"].unique())
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()
    ...
    for ax, n in zip(axes, n_values):
        ...
        for _, row in sub.iterrows():
            ...
            ax.plot(h5["generations"], h5["diversity"],
                    linestyle=ls, marker=mk, markersize=3,
                    color=colour, label=config_label(row), linewidth=1.2)
        ax.set_title(f"n = {n}", fontsize=11)
        ax.set_xlabel("Generation")
...
def main():
    ...
    parser.add_argument("--csv",     default="results/experiments_dim2.csv")
    ...
    # drop duplicate rows (same tag, re-run without clearing csv)
    ...
    plot_convergence(df, args.log_dir, args.out_dir)
    plot_diversity(df, args.log_dir, args.out_dir)
